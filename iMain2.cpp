...
void loadGameState() {
    ...
    if (inFile.is_open()) {
        std::string line;
        // Read Player Name
        std::getline(inFile, line); // "Player Name: [name]"
        if (line.rfind("Player Name: ", 0) == 0) { // Check if line starts with "Player Name: "
            std::string name_str = line.substr(13); // Extract name part
            strncpy(current_player_name, name_str.c_str(), 50);
            current_player_name[50] = '\0'; // Ensure null termination
            name_input_index = strlen(current_player_name);
        }

        // Read Health
        std::getline(inFile, line); // "Health: [value]"
        if (line.rfind("Health: ", 0) == 0) {
            playerhealth = std::stoi(line.substr(8));
        }

        // Read Score
        std::getline(inFile, line); // "Score: [value]"
        if (line.rfind("Score: ", 0) == 0) {
            gamescore = std::stoi(line.substr(7));
        }

        // Read My Ship Position
        std::getline(inFile, line); // "My Ship Position: ([x], [y])"
        ...
        // Read Enemy Count
        std::getline(inFile, line); // "Enemy Count: [value]"
...
void saveGameStateRealtime() {
    if (!isgamerunning || isgameover || isgamepaused) {
        return; // Only save if the game is actively running
    }

    std::ofstream outFile("gamestate.txt"); // Open in overwrite mode
    if (outFile.is_open()) {
...
void rungamelogicanddisplay()
{
    ...
    if (playerhealth <= 0 && !isgameover)
    {
        ...
        iPauseTimer(0); // Pause game timers
        if (!isscoresaved) { // Save score once when the screen appears
            saveScoreToFile();
            isscoresaved = true;
        }
        return;
    ...
    // Call the save function here to save game state every frame
    // WARNING: This can be I/O intensive and might affect game performance.
    // For better performance, consider using a timer (e.g., iSetTimer(1000, saveGameStateRealtime);)
    // in your main function instead of calling it every frame.
...
void showyouwonpage() {
    ...
    iSetColor(255, 255, 255); // Added instruction for 'b'
    iTextAdvanced(screenwidth / 2 - 250, screenheight / 2 - 100, "Press 'b' to go back to homepage.", 0.15, 1);

    // Removed the timer logic here
}
...
void iMouseMove(int mousex, int mousey)
{
    ...
    // New hover logic for resume options
    if (isresumegameactive) {
        // New Game button area (adjust coordinates to match your showResumeOptionsScreen)
        ...
        // Resume Button area (adjust coordinates to match your showResumeOptionsScreen)
        if (mousex >= screenwidth / 2 - 150 && mousex <= screenwidth / 2 + 150 &&
...
void iMouse(int button, int state, int mousex, int mousey)
{
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
    {
        ...
        if (ishomepageactive == true)
        {
            if (703<=mousex && mousex<=793 && 426<=mousey && mousey<=457) // Play button
            {
                // Check if gamestate.txt exists and has content
                ...
                if (fileExistsAndNotEmpty) {
                    ...
                    isresumegameactive = true; // Go to resume options screen
                } else {
                    ishomepageactive = false;
        ...
        // Handle clicks on the resume options screen
        if (isresumegameactive) {
            // New Game button area (adjust coordinates as needed for your UI)
            if (mousex >= screenwidth / 2 - 150 && mousex <= screenwidth / 2 + 150 &&
                mousey >= screenheight / 2 + 50 && mousey <= screenheight / 2 + 100) {
                isresumegameactive = false;
                isnameinputactive = true; // Go to name input for new game
                strcpy(current_player_name, ""); // Clear previous name
                name_input_index = 0; // Reset name input index
            }
            // Resume button area (adjust coordinates as needed for your UI)
            else if (mousex >= screenwidth / 2 - 150 && mousex <= screenwidth / 2 + 150 &&
                     mousey >= screenheight / 2 - 50 && mousey <= screenheight / 2 ) {
                isresumegameactive = false;
                loadGameState(); // Load game state from file
                isgamerunning = true; // Start the game
                ishomepageactive = false; // Ensure homepage is off
...
void iKeyPress(unsigned char key)
{
    ...
    else if (key == 'b' || key == 'B')
    {
        ...
        else if (isgamerunning == true && !isgameover && !isyouwonpageactive && !isgameoverpageactive) // Added win/game over page checks
        {
            isgamerunning = false;
            ishomepageactive = true;
            managemusicplayback();
        ...
        else if(isyouwonpageactive==true || isgameoverpageactive==true){ // Handle 'b' on win/lose screens
            isyouwonpageactive = false; // Deactivate win screen
            isgameoverpageactive = false; // Deactivate game over screen
            ishomepageactive = true; // Go to homepage
            resetallgamestates(); // Reset game state for a new game
