...
bool isresumeoptionsactive = false; // NEW: State for resume/new game screen
...
// Function Prototypes (Declarations) - ADDED
...
// New function to clear the gamestate.txt file
void clearGameStateFile() {
    std::ofstream outFile("gamestate.txt", std::ios::trunc); // Open in truncate mode
    if (outFile.is_open()) {
        outFile.close(); // Closing immediately truncates the file
    } else {
        std::cerr << "Error: Unable to open gamestate.txt for clearing." << std::endl;
    }
}

// Function to save game state to gamestate.txt - MODIFIED
void saveGameStateRealtime() {
    if (!isgamerunning || isgameover || isgamepaused) return; // Only save if game is running and not over/paused
    ...
    if (outFile.is_open()) {
        ...
        outFile << "Total Enemies Killed: " << totalenemieskilled << std::endl; // ADDED
        outFile << "Total Enemies Spawned: " << totaleniemiesspawned << std::endl; // ADDED
        ...
        for (int i = 0; i < currentactiveenemycount; ++i) {
            if (activeenemies[i].isactive) {
                outFile << "  - Type: " << (activeenemies[i].isbossenemy ? "Boss" : "Regular")
                        << ", Position: (" << activeenemies[i].positionx << ", " << activeenemies[i].positiony << ")"
                        << ", Health: " << activeenemies[i].currenthealth << std::endl;
            }
        }
        outFile.close();
...
// Function to load game state from gamestate.txt - MODIFIED
void loadGameState() {
    std::ifstream inFile("gamestate.txt");
    if (inFile.is_open()) {
        std::string line;
        std::string temp_name;
        char temp_char_name[51];
        int temp_health, temp_score, temp_enemy_count;
        double temp_playerx, temp_playery;
        int temp_totalenemieskilled, temp_totaleniemiesspawned; // ADDED

        std::getline(inFile, line); // Player Name
        sscanf(line.c_str(), "Player Name: %s", temp_char_name);
        strcpy(current_player_name, temp_char_name);

        std::getline(inFile, line); // Health
        sscanf(line.c_str(), "Health: %d", &temp_health);
        playerhealth = temp_health;

        std::getline(inFile, line); // Score
        sscanf(line.c_str(), "Score: %d", &temp_score);
        gamescore = temp_score;

        std::getline(inFile, line); // My Ship Position
        sscanf(line.c_str(), "My Ship Position: (%lf, %lf)", &temp_playerx, &temp_playery);
        playerx = temp_playerx;
        playery = temp_playery;

        // Load totalenemieskilled and totaleniemiesspawned - ADDED
        std::getline(inFile, line); // Total Enemies Killed
        sscanf(line.c_str(), "Total Enemies Killed: %d", &temp_totalenemieskilled);
        totalenemieskilled = temp_totalenemieskilled;

        std::getline(inFile, line); // Total Enemies Spawned
        sscanf(line.c_str(), "Total Enemies Spawned: %d", &temp_totaleniemiesspawned);
        totaleniemiesspawned = temp_totaleniemiesspawned;

        // Reset current enemies before loading to avoid duplicates
        currentactiveenemycount = 0;

        std::getline(inFile, line); // Enemy Count
        sscanf(line.c_str(), "Enemy Count: %d", &temp_enemy_count);

        std::getline(inFile, line); // "Active Enemies:" line

        for (int i = 0; i < temp_enemy_count; ++i) {
            if (std::getline(inFile, line)) {
                char enemy_type_str[20];
                double e_pos_x, e_pos_y;
                int e_health;
                sscanf(line.c_str(), "  - Type: %s, Position: (%lf, %lf), Health: %d", enemy_type_str, &e_pos_x, &e_pos_y, &e_health);

                if (currentactiveenemycount < MAX_ENEMIES) {
                    activeenemies[currentactiveenemycount].positionx = e_pos_x;
                    activeenemies[currentactiveenemycount].positiony = e_pos_y;
                    activeenemies[currentactiveenemycount].currenthealth = e_health;
                    activeenemies[currentactiveenemycount].isbossenemy = (strcmp(enemy_type_str, "Boss") == 0);
                    activeenemies[currentactiveenemycount].isactive = true;
                    activeenemies[currentactiveenemycount].movementspeed = activeenemies[currentactiveenemycount].isbossenemy ? 10 : 6;
                    activeenemies[currentactiveenemycount].targethorizontalposition = activeenemies[currentactiveenemycount].isbossenemy ? screenwidth - 550 : screenwidth - 450;
        ...
        iSetTimer(1000, saveGameStateRealtime); // Re-activate real-time saving
        managemusicplayback(); // Start game music
    } else {
        std::cerr << "No saved game state found or error opening gamestate.txt." << std::endl;
        ishomepageactive = true; // Go back to homepage if no save found
    }
}
...
void showResumeOptionsScreen() {
    ...
    iSetColor(255, 255, 0);
    iTextAdvanced(screenwidth / 2 - 350, screenheight / 2 + 150, "Resume Game or Start New?", 0.3, 2);

    iSetColor(255, 255, 255);
    iTextAdvanced(screenwidth / 2 - 250, screenheight / 2 + 50, "Press 'Y' to Resume", 0.2, 1);
    iTextAdvanced(screenwidth / 2 - 250, screenheight / 2 - 20, "Press 'N' to Start New Game", 0.2, 1);
}
...
void showyouwonpage() {
    ...
    if (gametickcount - endgametimercounter >= you_won_screen_duration_ticks) {
        ...
        clearGameStateFile(); // Clear saved game state after game end
...
void iDraw()
{
    ...
    else if (isresumeoptionsactive) // NEW CONDITION
    {
...
{
    if (isresumeoptionsactive) { // NEW: Handle resume/new game options
        if (key == 'y' || key == 'Y') {
            isresumeoptionsactive = false;
            loadGameState(); // Load existing game
        } else if (key == 'n' || key == 'N') {
            isresumeoptionsactive = false;
            ishomepageactive = true; // Go to homepage for new game
            clearGameStateFile(); // Clear saved state
            resetallgamestates(); // Reset game for new start
        }
...
int main(int argc, char *argv[])
{
    ...
    // Check for saved game state at startup - MODIFIED
    std::ifstream testFile("gamestate.txt");
    if (testFile.good()) {
        isresumeoptionsactive = true; // Activate resume options screen
        ishomepageactive = false; // Disable homepage
        testFile.close();
    } else {
        ishomepageactive = true; // Otherwise, go to homepage
    }
    ...
    iSetTimer(50, moveenemies);
    iSetTimer(2000, spawnnewenemies);
    iSetTimer(1000, saveGameStateRealtime); // Activate real-time saving
